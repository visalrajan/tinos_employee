...
class _TaskPageState extends State<TaskPage> {
  ...
  Future getDocs() async {
    ...
    userDetails.docs.forEach((element) {
      //map through each item in the task collection
      List<dynamic> members =
          element.get("members"); // Take member list from the collection task
      members.forEach((mbrElement) {
        // take member map/dictionary that contains name and email

        var newEmail = mbrElement["email"]; // take email from the dictionary
        if (userEmail == newEmail) {
          //check whether the current user email matches with any email from the task collection
          ...
          setState(() {
            ...
            userTask.add(
                userTaskMap); //add the task name which matches the email to a temporary list
          });
        }
      });
    });
